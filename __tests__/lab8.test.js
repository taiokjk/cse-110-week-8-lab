describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < prodItems.length; i++) {
      ...
      const button = await shadowRoot.$('button');
      await button.click();
      console.log(`Added product ${i + 1}/${prodItems.length}`);
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const button = await shadowRoot.$('button');
      const innerText = await (await button.getProperty('innerText')).jsonValue();
      console.log(`Checking product has been added to card ${i + 1}/${prodItems.length}`);
      expect(innerText).toBe('Remove from Cart');      
    }
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const button = await shadowRoot.$('button');
      await button.click();
      console.log(`Removed product ${i + 1}/${prodItems.length}`);
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      const button = await shadowRoot.$('button');
      const innerText = await (await button.getProperty('innerText')).jsonValue();
      console.log(`Checking product has been removed from card ${i + 1}/${prodItems.length}`);
